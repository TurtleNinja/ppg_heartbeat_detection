...
unsigned long timerStart = 0;              // timer for sending data once connected
...
void sendData()
{
  // get data from IMU and PPG
  if(getData()) {
    ppgValue = analogRead(ppgPin);
  
    // calculate the accelarometer L1-Norm
    ...
    char msg[22] = {0};
    sprintf(msg, "%8ld,%5ld,%5d;", timerStart, l1_norm, ppgValue);
...
void loop() {
  ...
  if (bleConnected) {
    if (micros() - timerStart >= sampling_period) {
      timerStart = micros();
      sendData();
    }
  }
